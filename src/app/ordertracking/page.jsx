...
function page() {
  ...
  const handleTrackOrder = async () => {
    ...
    try {
      const result = await action(API.TRACK_ORDER(orderId));
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 py-12 px-4">
        ...
        {error && <p className="text-center mt-8 text-red-600 font-semibold text-lg">Order not found</p>}
                    ...
                    {orderStatus.map((statusItem, index) => (
                        <div key={index} className="mb-10 relative flex items-start">
                            ...
                            <div className="ml-8 bg-gray-50 p-6 rounded-lg shadow-sm flex-1 transform transition duration-300 ease-in-out hover:scale-102 hover:shadow-md border border-gray-100">
                                <p className="font-bold text-xl text-gray-900 mb-1">{statusItem.status}</p>
                                <p className="text-sm text-gray-600 mb-2">{new Date(statusItem.added_on).toLocaleString()} &mdash; {statusItem.location}</p>
                                {/* Assuming 'description' is not available in new API response, removing it or handling gracefully */}
                                {/* <p className="text-gray-700 leading-relaxed">{statusItem.description}</p> */}
                            </div>
                        </div>
                    ))}
                </div>
